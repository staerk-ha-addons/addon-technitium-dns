...
readonly LOCK_FILE="/tmp/.${TOKEN_NAME}.lock"
...
load_saved_token() {
    ...
    if acquire_lock; then
        echo "${saved_token}"
        release_lock
        bashio::log.debug "Loaded saved API token"
        return 0
    fi
...
make_direct_call() {
    ...
    if [[ ${status} -eq 0 ]]; then
        if [[ "${response}" == *"\"status\":\"Error\""* ]]; then
            ...
            bashio::log.trace "Direct API call failed with error: ${error_msg}"
        else
            bashio::log.trace "Direct API call successful"
        fi
    else
        bashio::log.trace "Direct API call failed with curl exit code: ${status}"
    fi
